  ...
  render() {
    ...
    return (
      <Menu
        secondary
        borderless
        style={{ backgroundColor: admin && "#6582c5" }}
      >
        {admin && <Menu.Item className={custom_classes}>ADMIN</Menu.Item>}
        <Link to="/">
          <Menu.Item className={custom_classes}>Accueil</Menu.Item>
        </Link>
        {user && (
          <Link to="/admin/pages">
            <Menu.Item className={custom_classes}>Admin pages</Menu.Item>
          </Link>
        )}
        ...
        {user ? (
          ...
          <Menu.Menu position="right">
            <Menu.Item position="right" className={custom_classes}>
              <Link to="/sign_in">Connexion / Inscription</Link>
            </Menu.Item>
          </Menu.Menu>
        )}
      </Menu>
